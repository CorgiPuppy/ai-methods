...
#align(center, block[
	...
	=== 2.3.1. Исследование структуры данных
])
Ниже представлена техническая информация о структуре используемого датасета:
#block(fill: luma(240), inset: 10pt, radius: 5pt, width: 100%)[
...
#align(center, block[
	=== 2.3.2. Визуальный анализ (EDA)
])
Для каждого класса был построен "усредненный" шаблон цифры, позволяющий визуально оценить характерные черты начертания в выборке.
#figure(
	image("../lab2/assets/mean_images.png", width: 80%),
	caption: [Средние изображения цифр])

#align(center, block[
	=== 2.3.3. Влияние количества эпох на точность
])
В ходе эксперимента оценивалось влияние гиперпараметра «количество эпох» на итоговую точность классификации на тестовой выборке.
#figure(
	image("../lab2/assets/epochs_impact.png", width: 70%),
	caption: [Зависимость точности от числа эпох обучения]
)

#align(center, block[
	=== 2.3.4. Результаты и сравнение
])
...
Для детального анализа качества классификации были построены матрицы ошибок. Они показывают, какие цифры модели путают чаще всего.
#grid(
  columns: (1fr, 1fr),
  gutter: 10pt,
  figure(image("../lab2/assets/mlp_confusion.png"), caption: [Матрица ошибок MLP]),
  figure(image("../lab2/assets/cnn_confusion.png"), caption: [Матрица ошибок CNN])
)
*Детальный отчет классификации для MLP:*
#block(fill: luma(248), inset: 8pt, radius: 4pt, width: 100%)[
        #set text(size: 8pt, font: "Courier New")
        #raw(read("../lab2/assets/mlp_report.txt"))
]
*Детальный отчет классификации для CNN:*
#block(fill: luma(248), inset: 8pt, radius: 4pt, width: 100%)[
        #set text(size: 8pt, font: "Courier New")
        #raw(read("../lab2/assets/cnn_report.txt"))
]

#align(center, block[
	=== 2.3.6. Тестирование на пользовательских данных
])
Для проверки обобщающей способности моделей было проведено тестирование на 10 собственных рукописных цифрах, сфотографированных на камеру смартфона. Изображения прошли предварительную обработку: перевод в градации серого, инверсию цвета (для соответствия формату MNIST) и масштабирование до размера $28 times 28$ пикселей.
#figure(
  image("../lab2/assets/custom_test.png", width: 100%),
  caption: [Результаты распознавания пользовательских фотографий]
)
*Результаты теста:*
#block(fill: luma(245), inset: 10pt, radius: 5pt, width: 100%)[
  #set text(size: 9pt, font: "Courier New")
  #raw(read("../lab2/assets/custom_results.txt"))
]
